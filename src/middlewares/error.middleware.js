...
export const errorHandler = (err, req, res, next) => {
  ...
  if (!(error instanceof ApiError)) {
    if (error instanceof mongoose.Error.CastError) {
      ...
      error = new ApiError(HTTP_STATUS.BAD_REQUEST, MESSAGES.VALIDATION_FAILED, details);
    } else if (error.code === 11000) {
      error = new ApiError(HTTP_STATUS.CONFLICT, 'Duplicate value for a unique field');
    } else {
