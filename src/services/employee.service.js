...
export const createEmployee = async (payload) => {
  ...
  if (!user) {
    throw new ApiError(HTTP_STATUS.BAD_REQUEST, MESSAGES.REFERENCED_USER_NOT_FOUND);
  }

  const department = await Department.findById(departmentId);
  if (!department) {
    throw new ApiError(HTTP_STATUS.BAD_REQUEST, MESSAGES.REFERENCED_DEPARTMENT_NOT_FOUND);
  }

  if (await Employee.findOne({ user: userId })) {
    throw new ApiError(HTTP_STATUS.CONFLICT, MESSAGES.USER_HAS_EMPLOYEE_PROFILE);
  }

  if (await Employee.findOne({ employeeId })) {
    throw new ApiError(HTTP_STATUS.CONFLICT, MESSAGES.EMPLOYEE_ID_ALREADY_EXISTS);
  }
...
export const getEmployees = async ({ page = 1, limit = 10, status, department, search } = {}) => {
  const filter = {};
...
export const updateEmployee = async (id, payload) => {
  ...
  const { employeeId, department: departmentId, user: userId, ...rest } = payload;
